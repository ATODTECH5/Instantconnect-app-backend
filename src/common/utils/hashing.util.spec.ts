...
describe('hashing.util', () => {
	describe('hashSecret / verifySecret', () => {
		it('accepts the secret it hashed', async () => {
			...
			await expect(verifySecret(hash, 'Correct-Horse1')).resolves.toBe(
				true,
			);
		});
		...
		it('rejects a different secret', async () => {
			...
			await expect(verifySecret(hash, 'Correct-Horse2')).resolves.toBe(
				false,
			);
		});
		...
		it('returns false rather than throwing on a corrupt hash', async () => {
			await expect(verifySecret('not-a-hash', 'anything')).resolves.toBe(
				false,
			);
		});
	});
