...
export class AuthService {
  ...
  async verifyEmail(
    ...
    return this.startSession(await this.users.getByIdOrFail(user.id), context);
  }
  ...
  async verifyPasswordResetCode(dto: VerifyCodeDto): Promise<string> {
    ...
    const payload: PasswordResetPayload = { sub: user.id, codeId: record.id };
  ...
  private async verifyResetGrant(
    ...
    try {
      return await this.jwt.verifyAsync<PasswordResetPayload>(resetToken, {
        secret: this.config.passwordResetSecret,
      });
    } catch {
  ...
  private async startSession(
    ...
    const tokens: IssuedTokens = await this.tokens.issueSession(user, context);
  ...
  private async sendCode(
    ...
    if (purpose === VerificationPurpose.EmailVerification) {
      await this.mailer.sendEmailVerificationCode(user.email, firstName, code);
      return;
    }
