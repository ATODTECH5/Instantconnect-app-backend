...
describe('AuthService', () => {
  ...
  describe('register', () => {
    ...
    it('treats the international form as the same number', async () => {
      await service.register({ ...REGISTRATION, phone: '+2348012345678' });
    ...
    it('never stores the password itself', async () => {
      ...
      const calls = users.create.mock.calls as [{ passwordHash: string }][];
  ...
  describe('signIn', () => {
    ...
    it('gives the same answer for an unknown account as for a wrong password', async () => {
      ...
      await expect(service.signIn(credentials, CONTEXT)).rejects.toMatchObject({
  ...
  describe('verifyEmail', () => {
    ...
    it('refuses an address that is already verified', async () => {
      ...
      ).rejects.toMatchObject({ response: { code: 'EMAIL_ALREADY_VERIFIED' } });
    });
  ...
  describe('resetPassword', () => {
    it('rejects a grant that does not verify', async () => {
      ...
      await expect(
        service.resetPassword({ resetToken: 'forged', password: 'Password1' }),
