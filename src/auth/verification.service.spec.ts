...
describe('VerificationService', () => {
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        VerificationService,
        { provide: getRepositoryToken(VerificationCode), useValue: repository },
  ...
  describe('issue', () => {
    it('stores a hash the code verifies against, never the code itself', async () => {
      ...
      await expect(verifySecret(saved.codeHash, code)).resolves.toBe(true);
    });
  ...
  describe('verify', () => {
    ...
    it('returns the record when the code matches', async () => {
      const record = buildRecord({ codeHash: await hashSecret('123456') });
    ...
    it('locks the code out once the attempt budget is spent', async () => {
      repository.findOne.mockResolvedValue(
        buildRecord({ codeHash: await hashSecret('123456'), attempts: 5 }),
      );
