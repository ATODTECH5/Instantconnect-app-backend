...
describeWithDatabase('Auth (e2e)', () => {
  ...
  describe('POST /auth/register', () => {
    ...
    it('rejects an unknown field rather than ignoring it', async () => {
      const response = await register({ ...registration(), isAdmin: true });
    ...
    it('gives the same answer for an unknown account as for a wrong password', async () => {
      const [unknown, wrong] = await Promise.all([
        request(app.getHttpServer())
          .post('/api/v1/auth/sign-in')
          .send({ email: 'nobody@example.test', password: 'Password1' }),
        request(app.getHttpServer())
          .post('/api/v1/auth/sign-in')
          .send({ email: 'nobody2@example.test', password: 'Password2' }),
      ]);
